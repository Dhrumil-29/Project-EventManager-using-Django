...
# from home.models import sending
from home.models import Event_Registration
# from home.models import load
...
def login(request):
    if request.method == "POST": 
        ...
        if request.POST.get('person') == 'Host':
            # if  sending.objects.all().filter(email=hostid).exists() and sending.objects.get(email=hostid).password == password:
            if  Event_Registration.objects.all().filter(host_email=hostid).exists() and Event_Registration.objects.get(host_email=hostid).password == password:
                # context = {'event_list': sending.objects.all().filter(email=hostid)}
                context = {'event_list': Event_Registration.objects.all().filter(host_email=hostid)}
                return render(request, 'loginpage.html',context)  
            else:
                context = {'messages':"Username or password is incorrect !"}
                return render(request, 'loginpage.html', context)
        else:
            # if  load.objects.all().filter(email=hostid).exists() and load.objects.get(email=hostid).password == password:
            if  Participant_Registration.objects.all().filter(email=hostid).exists() and Participant_Registration.objects.get(email=hostid).password == password:
                # context = {'part_list': load.objects.all().filter(email=hostid)}
...
def event_detail(request):
    # event_list = sending.objects.all().filter(deadline__gte = datetime.now())
...
def event_registration(request):
    if request.method == "POST":
        ...
        # event = sending(nEvent=nEvent ,name=name, desc = desc, location=location,stime=stime,etime=etime,deadline=deadline, host_email=host_email,password=password)
        ...
        # if sending.objects.all().filter(host_email=host_email, nEvent=nEvent):
...
def participant_registration(request):
    if request.method == "POST":
        ...
        nevent = request.POST.get('nevent')
        # if load.objects.all().filter(nevent=nevent):
        ...
        # if load.objects.all().filter(email=email, nevent=nevent):
        if Participant_Registration.objects.all().filter(email=email, nevent=nevent):
            ...
            return render(request, 'Participant Registration.html', context)
        # event1 = load(fname=fname,lname=lname,num=num,email=email,password=password,nopar=nopar,nevent=nevent)
        event1 = Participant_Registration(fname=fname,lname=lname,num=num,email=email,password=password,nopar=nopar,nevent=nevent)
        # for i in sending.objects.all().filter(nEvent = nevent):
